...
export async function refreshTradeData() {
  ...
  try {
    ...
    return await buildStoreFromMonthly(monthlyResponses, {
      ...
      message:
        "已通过 KITA K-stat 官方公开 worker 更新月度 HS 品类出口金额、净重与单位价格；DATA_GO_KR_SERVICE_KEY 缺失或 KCS/data.go.kr 接口不可用时使用该路径。2026-07-06 复核：当前公开月度 HS 最新可落库期仍为 2026年5月；KCS/KDI 已公开 2026年6月全月海关暂定值，Korea.kr/MOTIE 仍提供 6月半导体与电脑/SSD proxy 精确品类金额。"
    });
  } catch (error) {
    const sample = buildSampleStore();
