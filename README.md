...
其中部分 1-20 日数据直接来自简报正文；只有在明确标为 `official_public_derived` 时才使用披露的总出口额和半导体占比推算。截至 2026-06-30，本地环境未配置 `DATA_GO_KR_SERVICE_KEY`，`npm run fetch` 会优先尝试 KITA K-stat 官方 worker 并已自动拉取 2024.12-2026.05 的 SSD HS `852351` 与 DRAM/HBM HS `854232` 月度出口金额、净重和推算单价；强制查询 2026.06 仍未返回可落库的当前年 HS 行。KCS TradeData 英文同源接口此前未返回 2026.05 行，因此本次月度 HS 最新值以 KITA K-stat 为准。TRASS 公开首页仅复核 6月1-20日总出口/进口暂定总量和 5月确报构建公告，未提供可公开落库的半导体/存储产品拆分。详细接口和判断方法保存在 [`docs/korea-trade-data-research.md`](docs/korea-trade-data-research.md)。
