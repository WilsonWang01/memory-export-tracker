...
其中部分 1-20 日数据直接来自简报正文；部分 1-10/1-20 数据由简报披露的总出口额和半导体占比推算，并在 `data/trade-data.json` 的 `source` 字段标为 `official_public_derived` 或 `official_public_reported`。截至 2026-06-13，本地环境未配置 `DATA_GO_KR_SERVICE_KEY`，`npm run fetch` 未走 data.go.kr Itemtrade API；此前无密钥调用返回 401 Unauthorized。已通过 KCS TradeData 官方公开页同源 JSON 和 KITA K-stat worker 复核 2025.01-2026.04 的 SSD HS `852351` 与 DRAM/HBM HS `854232` 月度出口金额、净重和推算单价。2026-06-13 复核时，KCS TradeData 同源 JSON 对本地直连返回访问拦截；KITA 公开页面 2026 年月份仍只显示 1-4，强制查询 2026.05 时返回 2025.05 的 `LAST_*` 对比列，但 `THIS_*` 当年列为 0、变化率 -100%，因此不能作为 2026 年 5 月 HS 当年明细落库。本次复核中 TRASS 公开首页仅复核 6月1-10日总出口/进口暂定总量和 4月确报构建公告，产品级暂定查询标为 premium，未提供公开半导体/存储拆分；KITA K-stat 仅用于公开 worker 复核，未新增 2026.05 HS 取值。详细接口和判断方法保存在 [`docs/korea-trade-data-research.md`](docs/korea-trade-data-research.md)。
